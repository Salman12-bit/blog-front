...
const Home = () => {
    ...
    // Fetch the role of the user from localStorage on component mount
    useEffect(() => {
    ...
    const ArticleList = async () => {
        try {
            ...
            toast.error("Error fetching articles:", { className: 'toast-custom' });
        }
    };
    ...
    return (
        ...
        <div className="background-radial-gradient overflow-hidden">
            ...
            <div className="container px-4 py-5 px-md-5 my-5" style={{ zIndex: 10 }}>
                <div className="row">
                    {userart.map((ele) => (
                        <div key={ele._id} className="product-grid col-xs-12 col-sm-6 col-md-4 col-lg-3">
            ...
            <ToastContainer />
            <Link to="/commmet-section" className="btn btn-danger mb-5">Comment Section</Link> <br />

            {nextAuth === 'admin' && (
                <Link to="/change-role"
                    className="btn btn-danger mb-5">Change Role
                </Link>
            )}
        </div>
